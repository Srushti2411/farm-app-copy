...
const ProductCart = ({ product }) => {
    ...
    // Event handler for updating product
    ...
    // Event handler for deleting product
    const handleDeleteProduct = async (pid) => {
        const { success, message } = await deleteProducts(pid);

        if (!success) {
            toast({
                title: "Error in deleting the product",
                description: message,
                status: "error",
                duration: 3000,
                isClosable: true
    ...
    return (
        ...
        >
            ...
            <Box p={4}>
                ...
                {/* Display Certified Farmer Badge */}
                {product.farmer?.isCertified && (
    <>
        <div className="badge badge-success text-sm text-green-500 font-semibold mt-2">
            ✔ Certified Farmer
        </div>
        {product.farmer?.certificate && (
            <a
                href={product.farmer.certificate}
                target="_blank"
